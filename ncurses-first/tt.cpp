...
void print_in_middle(int startx, int starty, int width, char *string,
                     WINDOW *win);
...
int print_menu() {
  ...
  while (1) {
    ...
    print_in_middle(1, 1, 0,
                    "* * *   Welcome to typing practice (Version 1.0) * * * ",
                    NULL);
    printw("\n\n\n");
...
void print_time(time_t start_t, time_t end_t, int mistakes) {
  ...
  mvprintw(STATUSY, STATUSX,
           "Mistakes made : %d time taken: %d:%d:%d WPM : %.2f    Press any "
           "Key to continue",
           mistakes, h, m, s, wpm);
  attroff(A_REVERSE);
