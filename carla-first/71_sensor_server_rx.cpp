...
auto make_camera_subscriber(std::string&& topic_name) {
  return [topic_name = std::move(topic_name)](boost::shared_ptr<csd::Image> image){
    ...
    if( _socket ) {
      ...
      asio::async_write(*_socket.get(), buffers, [frame, &topic_name](std::error_code ec, std::size_t length) {
        if (!ec) {
...
auto make_lidar_subscriber(std::string&& topic_name) {
  return [topic_name = std::move(topic_name)](boost::shared_ptr<csd::LidarMeasurement> lidar_data){
    ...
    if( _socket ) {
      ...
      asio::async_write(*_socket.get(), buffers, [topic_name, frame](std::error_code ec, std::size_t length) {
        if (!ec) {
          std::cout << std::this_thread::get_id() << " " << topic_name << " " << getEpochMillisecond() << " frame=" << frame
                    << " write len=" << length << std::endl;
        }
...
int main(int argc, const char *argv[]) {
  ...
  camera$.subscribe(make_camera_subscriber("/camera/0"));
  ...
  camera1$.subscribe(make_camera_subscriber("/camera/1"));
  ...
  camera2$.subscribe(make_camera_subscriber("/camera/2"));
  ...
  lidar$.subscribe(make_lidar_subscriber("/lidar/0"));
