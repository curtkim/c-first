...
int do_each_frame(void* encoder, NV_ENC_INPUT_PTR inputBuffer, NV_ENC_OUTPUT_PTR outputBuffer,
                  const int WIDTH, const int HEIGHT, const char * output_file) {
  ...
  for(int i = 0; i < 25*10; i++) {
    ...
    float v = i / 10.0f;
    glClearColor(0.0f, 0.0f, v - floor(v), 0.0f);
    glClear(GL_COLOR_BUFFER_BIT);

    int BUFFER_SIZE = WIDTH * HEIGHT * 4;
    unsigned char* imageData = (unsigned char *)malloc(BUFFER_SIZE);
    ...
    std::memcpy(inputBufferLocker.bufferDataPtr, imageData, BUFFER_SIZE);
    free(imageData);
    ...
    encodePicParam.bufferFmt = NV_ENC_BUFFER_FORMAT_ARGB;
    ...
    if( nvstatus == NV_ENC_ERR_NEED_MORE_INPUT){
      std::cout << i << " NV_ENC_ERR_NEED_MORE_INPUT" << std::endl;
      continue;
    }
    else if (nvstatus != NV_ENC_SUCCESS) {
      std::cout << "nvEncEncodePicture failed " << nvstatus << std::endl;
      return 1;
    }
    ...
    std::cout << i << " Encoded size: " << outputBufferLocker.bitstreamSizeInBytes << std::endl;
    ofs.write(
  ...
  std::cout << "ofs.close()" << std::endl;
}

int main() {
  const char * output_file = "target_egl.h264";
  ...
  // CHROMA_444 = 3,
  // NUM_CHROMA_FORMAT = 4
  encodeCfg.encodeCodecConfig.h264Config.chromaFormatIDC = 3;
  ...
  inputBufferParam.bufferFmt  = NV_ENC_BUFFER_FORMAT_ARGB;
  ...
  // 8 ~ 11
  int result = do_each_frame(encoder, inputBuffer, outputBuffer, WIDTH, HEIGHT, output_file);
  if( result > 0)
    return result;
