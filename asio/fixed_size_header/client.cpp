...
class Session;
using close_callback = std::function<void(std::shared_ptr<Session> ptr,
...
public:
  Session(int id, asio::io_context &io, const asio::ip::tcp::endpoint ep, close_callback cb)
      : id_(id), socket_(io), server_endpoint_(ep), wait_timer_(io), after_close_(cb) {}

  void Start() {
    auto self = shared_from_this();
    socket_.async_connect(
        server_endpoint_, [self, this](const asio::error_code &err) {
          if (err) // connect err
          {
          ...
          if (ec) {
            std::cerr << "client[" << id_
                      << "] set reuse error, message:" << ec.message() << "\n";
            return;
          }
  ...
  void SendQueuePkg() {
    ...
    if (!has_pkg) { // the queue is empty now , sleep for a while
      ...
      // send request to server, and then recv response
      asio::async_write(
          socket_,
          asio::buffer(&request_, sizeof(MyHeader) + request_.header.body_len),
          [self, this](const asio::error_code &err, size_t len) {
            ...
            // recv header first
            asio::async_read(
                socket_, asio::buffer(&response_, sizeof(MyHeader)),
                [self, this](const asio::error_code &err, size_t len) {
                  if (err) // read err
                  {
                  ...
                  if (response_.header.body_len > MAX_BODY_LEN) {
                    std::cerr
                        << "invalid body len: " << response_.header.body_len
                        << "\n";
                    Close(err);
                    return;
                  }
                  ...
                  asio::async_read(
                      socket_,
                      asio::buffer(&response_.body, response_.header.body_len),
                      [self, this](const asio::error_code &err, size_t len) {
                        if (err) {
...
private:
  ...
  close_callback after_close_; // when this session is close, maybe a new
                               // session should start, do it in this function
};
