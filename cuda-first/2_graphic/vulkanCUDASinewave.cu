...
private:
  ...
  void initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan-CUDA Interop Sinewave", nullptr, nullptr);
  }
...
#ifdef _WIN64
    fpGetMemoryWin32HandleKHR = (PFN_vkGetMemoryWin32HandleKHR)vkGetInstanceProcAddr(instance, "vkGetMemoryWin32HandleKHR");
  ...
  void createBufferExtMem(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
  ...
  void createVertexBuffer() {
    ...
    if (IsWindows8OrGreater())
        {
            createBufferExtMem(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT,
                vertexBuffer, vertexBufferMemory);
        }
        else
        {
            createBufferExtMem(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_KMT_BIT,
                vertexBuffer, vertexBufferMemory);
        }
#else
    createBufferExtMem(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT,
                       vertexBuffer, vertexBufferMemory);
#endif
  }
  ...
  void cudaInitVertexMem()
  {
    ...
    sinewave_gen_kernel<<<grid, block, 0, streamToRun>>>(vertices, mesh_width, mesh_height, 1.0);
  ...
  void createCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
      ...
      vkCmdDraw(commandBuffers[i], static_cast<uint32_t>(vertexBufSize), 1, 0, 0);
  ...
  void drawFrame() {
    ...
    // Added sleep of 5 millisecs so that CPU does not submit too much work to GPU
  ...
  void submitVulkan(uint32_t imageIndex)
  {
    ...
    VkSemaphore signalSemaphores[] = {renderFinishedSemaphore, vkUpdateCudaVertexBufSemaphore};
  ...
  void submitVulkanCuda(uint32_t imageIndex)
  {
    ...
    VkSemaphore waitSemaphores[] = {imageAvailableSemaphore, cudaUpdateVkVertexBufSemaphore};
  ...
  void createSyncObjectsExt() {
    ...
    vulkanExportSemaphoreCreateInfo.pNext       = IsWindows8OrGreater() ? &vulkanExportSemaphoreWin32HandleInfoKHR : NULL;
        vulkanExportSemaphoreCreateInfo.handleTypes = IsWindows8OrGreater() ? VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT : VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_KMT_BIT;
#else
...
#ifdef _WIN64
    cudaExtMemHandleDesc.type = IsWindows8OrGreater() ? cudaExternalMemoryHandleTypeOpaqueWin32 : cudaExternalMemoryHandleTypeOpaqueWin32Kmt;
        cudaExtMemHandleDesc.handle.win32.handle = getVkMemHandle(IsWindows8OrGreater() ? VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT : VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_KMT_BIT);
#else
    cudaExtMemHandleDesc.type = cudaExternalMemoryHandleTypeOpaqueFd;
    cudaExtMemHandleDesc.handle.fd = getVkMemHandle(VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT);
#endif
  ...
  void cudaVkImportSemaphore()
  {
    ...
    externalSemaphoreHandleDesc.type = IsWindows8OrGreater() ? cudaExternalSemaphoreHandleTypeOpaqueWin32 : cudaExternalSemaphoreHandleTypeOpaqueWin32Kmt;
        externalSemaphoreHandleDesc.handle.win32.handle = getVkSemaphoreHandle(IsWindows8OrGreater() ? VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT : VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_KMT_BIT, 
                                                                                cudaUpdateVkVertexBufSemaphore);
#else
    externalSemaphoreHandleDesc.type      = cudaExternalSemaphoreHandleTypeOpaqueFd;
    externalSemaphoreHandleDesc.handle.fd = getVkSemaphoreHandle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT, cudaUpdateVkVertexBufSemaphore);
#endif
...
#else
    externalSemaphoreHandleDesc.type      = cudaExternalSemaphoreHandleTypeOpaqueFd;
    externalSemaphoreHandleDesc.handle.fd = getVkSemaphoreHandle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT, vkUpdateCudaVertexBufSemaphore);
#endif
    externalSemaphoreHandleDesc.flags = 0;
  ...
  HANDLE getVkMemHandle(VkExternalMemoryHandleTypeFlagsKHR externalMemoryHandleType)
    {
        HANDLE handle;
  ...
  void cudaUpdateVertexBuffer()
  {
    ...
    AnimTime += 0.01f;
    sinewave_gen_kernel<<<grid, block, 0, streamToRun>>>(pos, mesh_width, mesh_height, AnimTime);
  ...
  void cleanup() {
    ...
    checkCudaErrors(cudaDestroyExternalSemaphore(cudaExtCudaUpdateVkVertexBufSemaphore));
