...
#include "shared/ImagesCPU.h"
#include "shared/ImagesNPP.h"
//#include "shared/ImageIO.h"
...
void loadImage(const std::string &rFileName, npp::ImageCPU_8u_C1 &rImage){
  int width;
  int height;
  int depth;

  unsigned char * image = stbi_load(rFileName.c_str(), &width, &height, &depth, 0);

  npp::ImageCPU_8u_C1 oImage(width, height);
  Npp8u *pDstLine = oImage.data();
  memcpy(pDstLine, image, width*height*1);
  oImage.swap(rImage);
}

void saveImage(const std::string &rFileName, const npp::ImageCPU_8u_C1 &rImage){
  stbi_write_jpg(rFileName.c_str(), rImage.width(), rImage.height(), 1, rImage.data(), 1);
}
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    sResultFilename += "_cannyEdgeDetection.jpg";
